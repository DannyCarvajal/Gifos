...
let inputsug = document.getElementsByClassName('inputsuggestion')[0]
...
function suggestionreq() {
    ...
    function erase(num = 0) {

        if (num == 1) {
            let xfade = document.getElementsByClassName('xfade')[0]
            borderinput.removeChild(xfade)
        }


        let resultactive = document.getElementsByClassName('resultactive')
        let navactive = document.getElementsByClassName('navactive')
        inputsug.value = ""

        for (let i = 0; i < resultactive.length; i++) {
            borderinput.removeChild(resultactive[i])
        }
        ...
        lupaactive.setAttribute('src', 'Assets/icon-search.svg')
